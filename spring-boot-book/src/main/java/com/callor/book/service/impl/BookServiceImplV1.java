...
public class BookServiceImplV1 implements BookService {
    ...
    public BookVO findById(String isbn) {
        ...
        /*
        JPA 의 findById() 는 type 이 Optional<T> 이다.
        Optional type은 데이터가 null 인 경우를 좀 더 쉽게 처리할 수 있도록 도와주는 도구이다
         */
        Optional<BookVO> opBookVO = bookDao.findById(isbn);

        /*
        Optional 데이터에서 실제 필요한 데이터 (bookVO) 를 추출하기 위하여 get() method 를 사용할 수 있는데
        보통은 orElse() 와 같은 method 를 사용하여
        만약 포함된 데이터가 (bookVO)가 null 일 경우 다른 값을 생성하여 bookVO 데이터가 절대 null 이 아니도록 처리할 수 있다
         */
        BookVO bookVO = opBookVO.ofElse(new BookVO()); // 혹은 BookVO bookVO = opBookVO.get();
