...
public interface BookService {
    ...
    null 값을 return 한다는 것은 잠재적으로 NullPointerException 을 일으킬 수 있다.
    
    보통 이 return 된 데이터를 취급할 때 값이 null 인가를 검사하는 코드가 산당히 번거롭게 많이 만들어져야 한다

    이러한 번거로움을 조금 감소하고 코드를 줄이기 위하여 null 값이 될만한 데이터는 Optional 로 감싸서 null 검사를 조금 쉽게 할 수 있도록 만들어 놓은 새로운 Type 이다.
